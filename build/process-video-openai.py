...
import whisper, time, sys, os, subprocess, torch, logging
...
def create_model():
  global model
  logging.info(f">>> Creating model = {MODEL_SIZE}, cache dir = /cache, device= {DEVICE}")
  ...
  logging.info(f">>> Model creation took {time.time() - start_time:.2f} seconds")
...
def transcribe_audio(audio_path, srt_path, video_path):
  ...
  logging.info(f">>> Transcribing : {video_path} -> {srt_path} ")
  start_time = time.time()
  transcription = model.transcribe(audio_path, fp16 = not DEVICE == "cpu")
  time_taken = time.time() - start_time
  logging.info(f">>> transcribing complete, time: {time_taken:.2f}, audio length: {audio_length:.2f}, ratio: {time_taken / audio_length:.2f}")
